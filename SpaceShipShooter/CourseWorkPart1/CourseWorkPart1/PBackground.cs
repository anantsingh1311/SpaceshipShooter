...
namespace CourseWorkPart1
{
    class PBackground
    {
        ...
        public void Initialize(ContentManager content, String texturePath, int screenWidth, int screenHeight, int speed)
        {
            ...
            pbTextures = content.Load<Texture2D>(texturePath);
            // Set the speed of the background               
            this.speed = speed;
            // If we divide the screen with the texture width then we can determine the number of tiles need.   
            // We add 1 to it so that we won't have a gap in the tiling   
            int numOfTiles = (int)(Math.Ceiling(screenWidth / (float)pbTextures.Width) + 1);
            positions = new Vector2[numOfTiles];
            // Set the initial positions of the parallaxing background               
            for (int i = 0; i < positions.Length; i++)
            {
                // We need the tiles to be side by side to create a tiling effect                   
                positions[i] = new Vector2(i * pbTextures.Width, 0);
            }
        }
        public void Update(GameTime gametime)
        {
            ...
            for (int i = 0; i < positions.Length; i++)
            {
                ...
                if (speed <= 0)
                {
                    ...
                    if (positions[i].X <= -pbTextures.Width)
                    {
                        WrapTextureLeft(i);
                    }
                }
                ...
                else
                {
                    ...
                    if (positions[i].X >= pbTextures.Width * (positions.Length - 1))
                    {
                        WrapTextureRight(i);
                    }
                }
        ...
        private void WrapTextureLeft(int index)
        {
            ...
            positions[index].X = positions[prevTexture].X + pbTextures.Width - 2;
        }

        private void WrapTextureRight(int index)
        {
            ...
            if (nextTexture == positions.Length)
                nextTexture = 0;
            positions[index].X = positions[nextTexture].X - pbTextures.Width + 2;
        }
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < positions.Length; i++)
            {
                ...
                spriteBatch.Draw(pbTextures, rectBg, Color.White);
            }
        }
    }
}
